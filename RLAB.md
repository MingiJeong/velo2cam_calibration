...
__NOTE__: 4 circles should be reflected and appear! 
...
```
* in case of launch file:
```
roslaunch camera_test test.launch (my laptop)
roslaunch usb_cam_node cam_node.launch (getac)
```

* Make sure you set up a proper parameter for camera (frame_id, video_device, format, autoexposure..).
e.g., rosparam set /usb_cam/video_device /dev/video1 (depending on `v4l2-ctl --device-list`)
...
```
* 1st step: The lidar_pattern `lidar_pattern_/zyx_filtered` cloud should represent only the __calibration pattern__ and the __points__ corresponding to those lidar rays that go through the holes. 
...
* 2nd step: On the other hand, `lidar_pattern_/range_filtered_cloud` must contain only the __calibration pattern__. To that end, a radial passthrough filter is available in the lidar_pattern_ node, tunable through the `passthrough_radius_min` and `passthrough_radius_max` parameters. Basically, this changes the ring part generated by __red__ while excluding __white__
...
Once you pass the camera and LiDAR matching step, 
...
## Run - offline method 
### 1. record a bag file
After the sensor drivers run, 
```
rosbag record /usb_cam_surface/image_raw /usb_cam_surface/camera_info /ouster/points
```

### 2. rosbag play
* play the bag file with -l

### 3. camera tab
* (in case the camera info was not published well) rosrun usb_cam_node yaml_to_camera_info_publisher.py
*  roslaunch velo2cam_calibration mono_pattern.launch camera_name:=/usb_cam_surface image_topic:=image_raw frame_name:=surface_cam camera_info_topic:=camera_info_test

I am supposed to see four green dots at the center of each hole; otherwise,there is something wrong, e.g., camera info is not published

### 4. LiDAR tab
* roslaunch velo2cam_calibration lidar_pattern.launch cloud_topic:=/ouster/points
* rosrun rqt_reconfigure rqt_reconfigure
Note that it is easy to do pass through first; then, plate only, radius second

### 5. calibration tab
* roslaunch velo2cam_calibration registration.launch sensor1_type:=mono sensor2_type:=lidar

Barbados 2025: 1 -> 3 -> 2
change loction ? Y -> bag play 

### 6. Run the tf launch
* Run the tf launch and get the extrinsic by `tf_echo`

### 7. obtain extrinsic matrix
* roslaunch overlay_sensor_fusion overlay_lidar.launch -- ext finder mode
